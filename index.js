...
const saveDataToMongoDB = require("./api/saveDataToMongoDB");
...
async function main() {
  ...
  if (round < taskData.maxRound) {
    ...
    const dataList = await queueCID(taskData.submissions);
    await saveDataToMongoDB(dataList);

    /*     // POST data to server
    let i = 0;
    let result = await queuePost(dataList, i); */
